...
class SpamDetector:
    ...
    def _generate_model_summary(self, features: List[Dict], prediction: str) -> str:
        """
        Generate a human-readable summary based on model features

        Args:
            features: List of feature explanations from model
        ...
        """
        if not features:
            return f"Classified as {prediction.upper()} with no significant features identified."

        top_features = features[:3]  # Top 3 features

        if prediction == 'spam':
            spam_features = [f['feature'] for f in top_features if f['direction'] == 'spam']
            if spam_features:
                return f"Classified as SPAM due to high-weight features: {', '.join(spam_features[:3])}"
            else:
                return "Classified as SPAM based on overall feature pattern."
        else:
            ham_features = [f['feature'] for f in top_features if f['direction'] == 'ham']
            if ham_features:
                return f"Classified as LEGITIMATE due to normal features: {', '.join(ham_features[:3])}"
            else:
                return "Classified as LEGITIMATE based on overall feature pattern."
